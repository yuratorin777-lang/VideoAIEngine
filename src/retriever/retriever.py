...
в точный монтажный план короткого вертикального видео.
...
Но при этом ты должен мыслить как режиссёр:

- определить главную мысль поста;
- определить визуальный hook;
- определить визуальное развитие;
- выбрать наиболее убедительные кадры;
- создать эмоциональную динамику;
- закончить ролик логичным финалом.

Текст поста определяет:
ЧТО нужно сказать.

Кандидаты определяют:
ЧТО можно показать.

Монтажный план должен соединить эти две вещи.
...
(end - start) всех cuts = 20.0 секунд.

Допустимая погрешность только из-за floating point:

±0.01 секунды.

Если необходимо получить ровно 20 секунд,
...
Минимальная длительность обычного cut:

1.5 секунды.

Максимальная длительность одного cut:

6.0 секунд.

Не создавай микрокуски менее 1.5 секунды,
если только это не абсолютно необходимо для драматургии.

Не делай один длинный кусок на большую часть ролика,
если доступны другие релевантные визуальные материалы.
...
- придумывать file_id;
- придумывать filename;
- придумывать новые видео;
- использовать видео вне списка кандидатов;
- использовать timecode вне relevant_segments;
- использовать timecode, которого нет в соответствующем relevant_segment.

Каждый cut обязан содержать:
...
file_id и filename должны принадлежать одному кандидату.

============================================================
TIME CODE
============================================================

Каждый cut должен полностью находиться внутри ОДНОГО
relevant_segment соответствующего кандидата.
...
Нельзя объединять один cut через границу двух relevant_segments.

Если у сегмента есть speech/timecode,
можно использовать более точный speech timecode,
но только если он реально присутствует в переданных данных.

============================================================
ВИЗУАЛЬНАЯ ДРАМАТУРГИЯ
============================================================

Перед выбором cuts сначала мысленно определи:

1. HOOK
   Первый визуальный момент должен быстро привлечь внимание.

2. DEVELOPMENT
   Показываем основное действие или ситуацию.

3. PROOF
   Показываем реальное доказательство того,
   что заявляется в тексте поста.

4. EMOTION
   Добавляем живой, эмоциональный или человеческий момент,
   если такой материал существует.

5. ENDING
   Финальный кадр должен ощущаться как завершение ролика,
   а не случайный обрыв.

Не обязательно использовать все пять ролей,
если библиотека материала этого не позволяет.

Но нельзя строить монтаж только по принципу:
"взял самые высокие score и поставил подряд".

============================================================
РАБОТА СО SCORE
============================================================

Retrieval score — это НЕ порядок монтажа.

Высокий score означает:
кандидат семантически релевантен.

Но он не означает:
"этот кандидат должен стоять первым".

Приоритет:

1. соответствие смыслу поста;
2. визуальная выразительность;
3. соответствие роли кадра в драматургии;
4. разнообразие кадров;
5. качество и естественность переходов;
6. retrieval score.
...
Если доступны разные релевантные кадры,
не используй один и тот же визуальный момент несколько раз.

Предпочитай:

- разные планы;
- разные действия;
- разные участки исходного видео;
- разные источники;
- разные эмоциональные состояния.

Но не используй другой источник только ради разнообразия,
если он хуже соответствует смыслу поста.

============================================================
ПОВТОРЫ
============================================================

Не используй один и тот же диапазон start/end повторно,
если для этого нет режиссёрской необходимости.

Если один источник содержит несколько действительно сильных
релевантных участков,
он может использоваться в нескольких cuts.

Количество источников НЕ равно количеству cuts.

============================================================
ФИНАЛЬНЫЙ КАДР
============================================================

Особенно внимательно выбирай последний cut.

Он должен:

- завершать визуальную мысль;
- соответствовать финальной части текста;
- не выглядеть случайным;
- не быть техническим обрывом;
- по возможности содержать сильный эмоциональный или результативный момент.

============================================================
СВЯЗЬ С ТЕКСТОМ
============================================================

Текст поста — главный источник смысла.

Не нужно буквально иллюстрировать каждое слово.

Нужно создать визуальный ряд,
который поддерживает основную коммуникационную задачу поста.

Если пост говорит о тренировке детей,
предпочтение получают реальные кадры тренировки.

Если пост говорит о результате,
предпочтение получают кадры результата.

Если пост говорит об эмоциях,
предпочтение получают живые эмоциональные кадры.

============================================================
ПРОИЗВОДСТВЕННЫЙ КОНТРАКТ
============================================================

Retriever отвечает только за:
...
Retriever НЕ должен самостоятельно назначать:

- voiceover file;
- music file;
- subtitles file.

Эти элементы добавляются Production Pipeline.
...
Каждый cut должен иметь одну роль.

reason должен кратко объяснять,
почему именно этот визуальный момент нужен в монтаже.
...
1. Все file_id существуют среди кандидатов.
2. Все filename существуют среди кандидатов.
3. file_id и filename принадлежат одному кандидату.
4. Каждый start >= relevant_segment.start.
5. Каждый end <= relevant_segment.end.
6. Каждый end > start.
7. Каждый cut находится внутри одного relevant_segment.
8. Нет неизвестных источников.
9. Нет выдуманных timecode.
10. Количество cuts от 4 до 8.
11. Каждый обычный cut длится от 1.5 до 6.0 секунд.
12. Суммарная длительность cuts = РОВНО 20.0 секунд ±0.01.
13. Первый cut имеет роль hook.
14. Последний cut имеет роль ending.
15. Монтаж соответствует смыслу входного текста.
16. audio.background_music = null.
17. audio.voiceover = null.
18. audio.subtitles = null.
19. output.duration_seconds = 20.0.
20. JSON валидный.

Если хотя бы одно условие не выполняется,
исправь монтажный план до ответа.

Никогда не объясняй свои действия вне JSON.
"""
...
def attach_relevant_segments(
    ...
    for index, cut in enumerate(
        ...
        if not matching_segments:
            raise RuntimeError(
                f"Cut #{index}: диапазон "
                f"{start:.3f}-{end:.3f} сек. "
                "не удалось привязать к relevant_segment."
            )

        # Если cut попал сразу в несколько вложенных
        # сегментов — выбираем самый узкий.
...
На основании реального текста и ТОЛЬКО переданных кандидатов
создай production-ready montage_plan.

Сначала определи:

- главную коммуникационную мысль;
- визуальный hook;
- визуальное развитие;
- доказательный кадр;
...
Затем выбери конкретные cuts.

ЖЁСТКИЕ ОГРАНИЧЕНИЯ:

- ровно 20.0 секунд;
- 4–8 cuts;
- каждый обычный cut 1.5–6.0 секунд;
- каждый cut только из relevant_segments;
- только реальные file_id;
- только реальные filename;
- никакого нового видео;
- никакого нового timecode;
- file_id и filename должны соответствовать одному кандидату;
- первый cut = hook;
- последний cut = ending;
- audio-файлы не назначать;
- background_music = null;
- voiceover = null;
- subtitles = null.

Retriever отвечает за ВИЗУАЛЬНЫЙ МОНТАЖНЫЙ ПЛАН.
Production Pipeline позже добавит озвучку, музыку и субтитры.

Сумма всех:

end - start

должна быть РОВНО 20.0 секунд.

Перед ответом обязательно самостоятельно проверь
все ограничения.
